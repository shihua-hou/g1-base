...
def capture_snapshot(session, out_path, group="upper"):
    ...
    try:
        ...
        snapshot = save_snapshot(out_path, group=group, joints=joints, q=q_save)
    ...
    try:
        ...
        if _can_play_arm_sequence(session, debug_state):
            ...
            for step in range(steps):
                ...
                q_cmd_local, dq_cmd_local, _alpha = _compute_snapshot_trajectory_point(
                    source_q_cmd,
                    q_target,
                    raw,
                    duration,
            ...
            kp_vec = sequence_result.get("kp") or _build_joint_gain_vector(
                command_joints,
                kp,
                waist_support_kp,
            )
            kd_vec = sequence_result.get("kd") or _build_joint_gain_vector(
                command_joints,
                kd,
                waist_support_kd,
            )
        ...
        if source_result is None:
            ...
            for _ in range(takeover_steps):
                if cancel_event and cancel_event.is_set():
                    print("[SNAPSHOT] 收到取消信号，中断 takeover hold")
                    break
        ...
        for step in range(steps):
            if cancel_event and cancel_event.is_set():
                print("[SNAPSHOT] 收到取消信号，中断 snapshot")
                break
