"""Sequential script runner for snapshot/motion/hold/hand composition.

Scripts are treated as a blocking event stream: resolve each step in order,
execute snapshot/motion/hold/hand, or recursively expand nested scripts.
"""
...
ALLOWED_STEP_TYPES = {"snapshot", "motion", "hold", "script", "hand"}
...
def load_script(path):
    ...
    if path.suffix.lower() == ".jsonl":
        return path, {"profile": "upper_waist_lock"}, [{"type": "motion", "path": str(path)}]
...
def run_script(
    ...
    base_dir = Path(movement_dir) if movement_dir else script_path.parent
    ...
    try:
        for step_index, step in enumerate(steps, start=1):
            if cancel_event and cancel_event.is_set():
                print(f"{indent}[SCRIPT] 收到取消信号，退出脚本")
                break
            repeat = int(step.get("repeat", 1))
            for repeat_idx in range(repeat):
                    ...
                    if dry_run:
                        ...
                        if last_result is None:
                            ...
                            while time.time() < deadline:
                                if cancel_event and cancel_event.is_set():
                                    print(f"{indent}[SCRIPT] 收到取消信号，中断 hold")
                                    break
                                time.sleep(min(0.05, max(0.0, deadline - time.time())))
                        else:
                            hold_dt = float(last_result.get("control_dt", 0.01))
                            hold_steps = max(1, int(duration / hold_dt))
                            if callable(getattr(session, "play_arm_sequence", None)):
                                frames = [
                                    {
                                        "q": last_result["q_final"],
                                        "kp": last_result["kp"],
                                        "kd": last_result["kd"],
                                    }
                                    for _ in range(hold_steps)
                                ]
                                hold_result = session.play_arm_sequence(
                                    last_result["joints"],
                                    frames,
                                    dt=hold_dt,
                                    release=False,
                                ...
                                last_result = {
                                    "joints": list(hold_result["joints"]),
                                    "q_final": list(hold_result["q_final"]),
                                    "kp": hold_result["kp"],
                                    "kd": hold_result["kd"],
                                    "control_dt": float(hold_result.get("control_dt", hold_dt)),
                                }
                            else:
                                for _ in range(hold_steps):
                                    if cancel_event and cancel_event.is_set():
                ...
                # Every non-hold/non-hand step resolves to a concrete file path.
